...
public class JwtUtils {
    ...
    public String generateJwtToken(Authentication authentication) {
        ...
        Optional<User> userWhoRequestForAToken = userRepository.findById( userPrincipal.getId() );

        Date now = new Date();
        Date expirationDate = new Date(now.getTime() + jwtExpirationMs);

        String tokenStrValue = Jwts.builder()
        ...
        if(userWhoRequestForAToken.isPresent()) {
            UserToken userToken = new UserToken(
                    userWhoRequestForAToken.get(),
                    tokenStrValue,
                    true,
                    now,
                    expirationDate
